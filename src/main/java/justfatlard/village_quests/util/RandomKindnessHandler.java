...
public class RandomKindnessHandler {
   public static boolean tryRandomKindness(ServerPlayer player, Villager villager, String villagerName, int reputation, boolean isChild) {
      ...
      if (isChild) {
         if (reputation >= 25 && !(rng.nextFloat() >= 0.1F)) {
            ...
            ItemStack gift = gifts[rng.nextInt(gifts.length)];
            player.sendSystemMessage(Component.literal(villagerName + ": \"" + msg + "\"").withStyle(ChatFormatting.GREEN), false);
            player.getInventory().add(gift.copy());
            return true;
         } else {
            return false;
         }
         ...
         if (reputation >= 200 && rng.nextFloat() < 0.6F) {
            ...
            if (isPoisoned || isWithered) {
               ...
               player.sendSystemMessage(
                  Component.literal(villagerName + ": \"" + msgs[rng.nextInt(msgs.length)] + "\"").withStyle(ChatFormatting.GREEN), false
               );
